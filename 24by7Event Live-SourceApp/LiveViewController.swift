...
final class LiveViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        Event24By7.shared.broadcastSetUp("demo24@gmail.com") { response in
            if let dataDO = response?["data"] as? [String: Any] , let url = dataDO["rtmpUrl"] as? String ,  let key = dataDO["rtmpStreamKey"] as? String{
                LiveRTMP.defaultInstance.url = url
                LiveRTMP.defaultInstance.streamName = key
            }else{
    ...
    @IBAction func on(publish: UIButton) {
        if publish.isSelected {
            ...
            rtmpConnection.connect(LiveRTMP.defaultInstance.url!)
            publish.setTitle("■", for: [])
        }
    ...
    private func rtmpStatusHandler(_ notification: Notification) {
        ...
        case RTMPConnection.Code.connectSuccess.rawValue:
            retryCount = 0
            rtmpStream.publish(LiveRTMP.defaultInstance.streamName!)
...
struct LiveRTMP {
    static var defaultInstance = LiveRTMP()
    var url: String? = ""
